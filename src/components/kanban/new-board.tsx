...
const NewBoard = ({ board, setShowModal }: NewBoardProps) => {
    ...
    board
      ...
      : {
          ...
          columns: [
            ...
            { name: "In Progress", id: undefined },
            { name: "Done", id: undefined },
          ],
        }
  )

  const { dispatch } = useContext(AppStateContext)
  const mutation = api.kanban.editBoard.useMutation({
    onSuccess: (data) => {
      dispatch({
  ...
  const newBoardMutation = api.kanban.createBoard.useMutation({
    onSuccess: (data) => {
